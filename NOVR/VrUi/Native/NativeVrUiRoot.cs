...
public class NativeVrUiRoot : NOVRBehaviour
{
    ...
    private bool _stockOverlayPassthrough;
    ...
    {
        _stockOverlayPassthrough = false;
    ...
    private void OnNativeActionInvoked(NativeGameAction action)
    {
        ...
        if (IsStockOverlayAction(action))
        {
            _stockOverlayPassthrough = true;
            RestoreOriginalMainCanvas();
            if (_root != null)
            {
                _root.SetActive(false);
    ...
    private static bool IsStockOverlayAction(NativeGameAction action)
    {
        return action is NativeGameAction.Encyclopedia
            or NativeGameAction.ChangeLog
            or NativeGameAction.ControlChanges
            or NativeGameAction.DevelopmentRoadmap
            or NativeGameAction.Community
            or NativeGameAction.MissionEditor;
    }
