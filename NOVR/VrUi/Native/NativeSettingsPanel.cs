...
public class NativeSettingsPanel : MonoBehaviour
{
    ...
    private void RenderControlsTab()
    {
        ...
        AddFloatRow("VR Zoom Speed", () => ModConfiguration.Instance.ZoomSpeed.Value, value => SaveVrZoomFloat(ModConfiguration.Instance.ZoomSpeed, value), 0.1f, 20f, 0.1f, value => $"{value:0.0}x/s");
        AddFloatRow("VR Maximum Zoom", () => ModConfiguration.Instance.MaximumZoom.Value, value => SaveVrZoomFloat(ModConfiguration.Instance.MaximumZoom, value), 1f, 10f, 0.25f, value => $"{value:0.##}x");
        AddToggleRow("VR Instant Zoom Out", () => ModConfiguration.Instance.InstantZoomOut.Value, value => SaveVrZoomBool(ModConfiguration.Instance.InstantZoomOut, value));
    ...
    private static void SaveVrZoomFloat(ConfigEntry<float> entry, float value)
    {
        entry.Value = value;
        ModConfiguration.Instance.Config.Save();
    }

    private static void SaveVrZoomBool(ConfigEntry<bool> entry, bool value)
    {
        entry.Value = value;
        ModConfiguration.Instance.Config.Save();
    }
